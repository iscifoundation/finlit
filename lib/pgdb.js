...
class Collection {
  ...
  async updateOne(filter, update, options = {}) {
    ...
    const where = buildWhere(filter, params);
    // Grab the target row id (limit 1)
    const idParams = params.slice();
    const findSql = `SELECT id FROM ${this._table()}${where.sql} LIMIT 1`;
    const found = await this._q(findSql, idParams);
    if (found.rows.length === 0) {
      if (options.upsert) {
        // Build a doc by applying update ops on an empty object.
        const emptyId = String(filter.id ?? crypto.randomUUID?.() ?? Date.now());
        // Apply $set fields directly, ignore other ops for simplicity
    ...
    const idxParam = params.length;
    // Re-run the WHERE with just id to be safe & concurrent-friendly
    const upSql = `UPDATE ${this._table()} SET doc = ${expr} WHERE id = $${idxParam}`;
    const r = await this._q(upSql, params);
